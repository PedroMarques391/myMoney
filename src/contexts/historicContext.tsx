...
function HistoricProvider({children}: HistoricProviderProps) {
  ...
  async function filter(
    selectedOption: TOptionsTypes,
    historicData: IHistoricalItemInterface[],
  ): Promise<void> {
    if (selectedOption === 'today') {
      return setHistoric(
        historicData.filter(item => item.date === dayjs().format('DD/MM/YYYY')),
      );
    }
    if (selectedOption === 'revenue') {
      return setHistoric(
        historicData.filter(item => item.type === 'receita').reverse(),
      );
    }
    if (selectedOption === 'expenses') {
      return setHistoric(
        historicData.filter(item => item.type === 'despesa').reverse(),
      );
    }
  }
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const q = await query(dataRef, orderByChild('date'), limitToLast(10));
      ...
      const getData = onValue(q, async snapshot => {
        ...
        const expenses = historicData
          .filter(item => item.type === 'despesa')
          .reduce((acc, item) => acc + item.value, 0);
        setTotalExpenses(expenses);

        const revenue = historicData
